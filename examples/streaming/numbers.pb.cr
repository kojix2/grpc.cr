# Proto stubs for the Numbers service.
...
module Numbers
  ...
  SERVICE_NAME = "numbers.Numbers"

  abstract class NumbersService < GRPC::Service
  ...
  class NumbersClient
    ...
    def sum(ctx : GRPC::ClientContext = GRPC::ClientContext.new) : GRPC::ClientStream(Number, Number)
      ...
      GRPC::ClientStream(Number, Number).new(send_proc, close_proc, result_chan, -> { raw.status }, -> { raw.trailers }, -> { raw.cancel })
    end
    ...
    def transform(ctx : GRPC::ClientContext = GRPC::ClientContext.new) : GRPC::BidiCall(Number, Number)
      raw = @channel.open_bidi_stream_live(SERVICE_NAME, "Transform", ctx)
